...
- Deterministic troubleshooting runbook recommendations based on incident category

## Incident Analysis Flow

1. The user describes an incident.
2. The application classifies the incident category using deterministic rules.
3. The application determines the incident severity.
4. A category-based troubleshooting runbook is recommended.
5. The user selects a sample log file.
6. The application parses the log and extracts `ERROR` entries.
7. Incident metadata and parsed errors are stored in Elasticsearch.
8. The incident details and error evidence are sent to Gemini.
9. Gemini generates AI-assisted investigation insights and recommended actions.
10. Indexed incident data can be explored in Kibana.
...
The application provides deterministic troubleshooting runbooks based on the classified incident category.

Runbooks provide predictable operational guidance for common incident types such as:
...
The runbook is intentionally rule-based rather than generated by the LLM. This keeps the core troubleshooting guidance predictable, while Gemini is used to provide additional context and AI-assisted investigation insights.
